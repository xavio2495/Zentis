...
describe('sim-report renders the committed run, and nothing else', () => {
	...
	test('a row carries mean, deviation, worst and the seeds-ahead ratio verbatim', () => {
		const report: Report = {
			...
			unit: 'raw tokenA',
			regimes: [
				{ regime: 'trend', trendBps: 300, volBpsPerTick: 25, seeds: 24, meanVsStatic: 178618, sdVsStatic: 19621, worstVsStatic: 151256, seedsAhead: 24, meanBookDrift: 0.0139 },
			],
		}
		expect(renderMarkdown(report)).toContain('| trend | 300 bps | 25 bps | 178,618 | 19,621 | 151,256 | 24/24 | 0.0139 |')
	})
})
