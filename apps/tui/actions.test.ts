...
// variable it wants, so a name a shell could not take is not a hazard any more. See sign.test.ts.

test("the fill is built from the deployment record, not from numbers typed here", () => {
  const intent = JSON.parse(find(envPath, "f").command!.stdin!) as { chainId: number; amount: string };
  expect(intent.chainId).toBe(LEGS[0]!.chainId);
  // The size is the book's own quote size, which is configured rather than chosen here.
  expect(intent.amount).toBe(String(QUOTE_SIZE_A));
});
...
test("a typed fill names its own leg, side and size", () => {
  ...
  const intent = JSON.parse(action.command!.stdin!) as { chainId: number; amount: string; isAToB: boolean };
  expect(intent.chainId).toBe(leg.chainId);
  expect(intent.amount).toBe("250000");
  expect(intent.isAToB).toBe(false);
});
...
// What a fill encodes — the recorded order, the side's taker data, the router's own selector — is
// pinned in intents.test.ts, where it is checked by decoding the call rather than by reading a shell
// command. What the quote-before-and-after sequence does is pinned in sign.test.ts against a
...
// The typed quote is no longer an action at all: it is an `eth_call` this process makes, covered by
// intents.test.ts for what it encodes and by quote.ts for what it decodes.
...
test.skip("the fill sources the env file inside the child, skipping names a shell cannot take", () => {
...
// The typed quote is no longer an action: it is an `eth_call` this process makes. What it encodes
// is pinned in intents.test.ts, and whether a leg can be quoted at all is `quoteAvailability`.
test("a leg with no recorded bytes cannot be quoted, and says why rather than guessing an order", () => {
  expect(quoteAvailability({ ...LEGS[0]!, fill: null })).toContain("no fill bytes recorded");
  expect(quoteAvailability(LEGS[0]!)).toBeNull();
});
...
test("no quote and no fill reaches for cast any more", async () => {
  // Step 1 of plan 11. The push still shells out and is step 2, which is what the gate there is for:
  // `grep -r cast apps/tui/src` empty, and this assertion widened to the whole file.
  // Code, not prose: the comments say what these calls used to be, which is worth keeping.
  const source = (await Bun.file(new URL("./src/actions.ts", import.meta.url)).text())
    .split("\n")
    .filter((line) => !line.trim().startsWith("*") && !line.trim().startsWith("//"))
    .join("\n");
  expect(source).not.toContain("cast call");
  expect(source).not.toMatch(/cast send \$\{token\.address\} "approve/);
  const fill = buildFillAction("/tmp/private.env", null, { leg: LEGS[0]!, amountRaw: 1n, isAToB: true });
  expect(JSON.stringify(fill.command)).not.toContain("cast");
});
